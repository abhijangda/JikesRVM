...
public final class RVMThread extends ThreadContext {
	 ...
	 * are only used deep within the stack. Note: If you change the assignments,
	 * update READABLE_EXEC_STATUS to ensure correct debug output.
	 */
	/**
	...
	private static final String[] READABLE_EXEC_STATUS = { "NEW", "IN_JAVA",
			"IN_NATIVE", "IN_JNI", "IN_JAVA_TO_BLOCK", "BLOCKED_IN_NATIVE",
			"BLOCKED_IN_JNI", "TERMINATED", "LAST_EXEC_STATUS" };
	...
	@Entrypoint
	private final AbstractRegisters trampolineRegisters = ArchitectureFactory
			.createRegisters();
	...
	private Address hijackedReturnCallerFp = StackFrameLayout
	...
	private boolean attemptFastExecStatusTransition(int oldState, int newState) {
		if (Synchronization.tryCompareAndSwap(this,
				Entrypoints.execStatusField.getOffset(), oldState, newState)) {
	...
	/**
	 * Should the thread block so that another thread can get a stack trace for
	 * it?
	 */
	boolean shouldBlockForStackTrace;

	/**
	 * Is the thread blocked because another thread wants to get a stack trace
	 * for it?
	 */
	...
	public abstract static class BlockAdapter {
		...
		/**
		 * Specifies that the thread is either blocked {@code (value == true)}
		 * or not blocked {@code (value == false)} for this block adapter. This
		 * call indicates a statement of fact by the thread itself - it's used
		 * either to acknowledge a block request (see
		 * {@link #hasBlockRequest(RVMThread)} below) or to respond to a request
		 * to unblock.
		 * 
		 * @param t
		 *            the thread
		 * @param value
		 *            the new value of the status for blocking as described
		 *            above
		 */
		abstract void setBlocked(RVMThread t, boolean value);

		/**
		 * Requests that the thread block, for this block adapter, at its
		 * earliest convenience.
		 * <p>
		 * Called from RVMThread.block() and associated methods. Some block
		 * adapters allow for multiple requests to block; in that case this will
		 * return a "token" that can be passed to hasBlockRequest() to check,
		 * not only whether there is a block request, but whether that block
		 * request is still associated with a particular call to requestBlock().
		 * This is used to prevent a suspend() call from stalling due to a
		 * concurrent resume() and second suspend(). Note that most block
		 * adapers don't care about this scenario, and will just return 0 (or
		 * some other meaningless number) here.
		 *
		 * @param t
		 *            the thread that needs to block
		 ...
		 * @param t
		 *            the thread
		 * @return whether the thread has a request to block for this block
		 *         adapter
		 */
		...
		/**
		 * @param t
		 *            the thread to check for block requests
		 * @param token
		 *            a token, see {@link #requestBlock(RVMThread)}
		 * @return whether the thread has a block request associated with the
		 *         given requestBlock() call
		 */
	...
	public static class ThreadStackTraceBlockAdapter extends BlockAdapter {
		...
		boolean hasBlockRequest(RVMThread t, int token) {
			return t.shouldBlockForStackTrace
					&& t.shouldBlockForStackTraceToken == token;
		}
	 ...
	 * should not be taken. &gt;0 means that the next yieldpoint of any type
	 * should be taken &lt;0 means that the next prologue/epilogue yieldpoint
	 * should be taken
	 * <p>
	...
	/**
	 * Thread priority handle. Used when manipulating the threads priority. This
	 * may be different from pthread_id.
	 */
	public Word priority_handle;

	/**
	 * Scratch area for use for gpr &lt;=&gt; fpr transfers by PPC baseline
	 * compiler. Used to transfer x87 to SSE registers on IA32
	 */
	@SuppressWarnings({ "unused" })
	 ...
	 * <li>numThreads, numActiveThreads, numActiveSystemThreads,
	 * numActiveDaemons static fields of RVMThread</li>
	 * </ul>
	 */
	...
	/**
	 * Number of active system threads. Necessary for JMX because it's better to
	 * not count system threads to be consistent with other VMs.
	 */
	...
	public static void init() {
		...
		BootRecord.the_boot_record.dumpStackAndDieOffset = Entrypoints.dumpStackAndDieMethod
				.getOffset();
		Lock.init();
	}
	...
	public void assertUnacceptableStates(int unexpected1, int unexpected2) {
		if (VM.VerifyAssertions) {
			...
			if (curStatus == unexpected1 || curStatus == unexpected2) {
				VM.sysWriteln(
						"FATAL ERROR: unexpected thread state for thread",
						threadSlot);
	...
	public static void boot() {
		...
		if (traceAcct) {
			VM.sysWriteln("boot thread at ",
					Magic.objectAsAddress(getCurrentThread()));
		}
	...
	void assignThreadSlot() {
		...
		if (traceAcct) {
			VM.sysWriteln("Thread #", threadSlot, " at ",
	...
	void releaseThreadSlot() {
		...
		Magic.sync(); /*
					 ...
					 * then they see the replacementThread moved to the new
					 * index before they see the numThreads decremented
					 * (otherwise they would miss replacementThread; but with
					 * the current arrangement at worst they will see it twice)
					 */
	...
	public RVMThread(byte[] stack, Thread thread, String name, boolean daemon,
			SystemThread systemThread, int priority) {
		this.stack = stack;
		this.daemon = daemon;
		this.priority = priority;
		this.systemThread = systemThread;

		this.contextRegisters = this.contextRegistersShadow = ArchitectureFactory
				.createRegisters();
		this.contextRegistersSave = this.contextRegistersSaveShadow = ArchitectureFactory
				.createRegisters();
		this.exceptionRegisters = this.exceptionRegistersShadow = ArchitectureFactory
				.createRegisters();
		...
		if (!VM.runningVM) {
			...
			stackLimit = Magic.objectAsAddress(stack).plus(
					StackFrameLayout.getStackSizeGuard());
			...
			if (thread == null) {
				...
				this.thread = java.lang.JikesRVMSupport
						.createThread(this, name);
			}
		}
	}

	/**
	 * Creates a thread with default stack and with the given name. The thread
	 * will be a daemon thread that runs at normal priority and is not
	 * associated with a {@link Thread} object.
	 *
	 * @param systemThread
	 *            the associated system thread
	 * @param name
	 *            human-readable name
	 */
	public RVMThread(SystemThread systemThread, String name) {
		this(MemoryManager.newStack(StackFrameLayout.getStackSizeNormal()),
	 ...
	 */
	public RVMThread(Thread thread, long stacksize, String name,
			boolean daemon, int priority) {
		this(MemoryManager.newStack((stacksize <= 0) ? StackFrameLayout
				.getStackSizeNormal() : (int) stacksize), thread, name, daemon,
				null, priority);
	}
	...
	/**
	 * Checks whether the thread is in native code as understood by the JMX
	 * ThreadInfo. A thread is considered in native if it is executing JNI code.
	 * <p>
	 * Note: this method is NOT designed for internal use by the RVMThread class
	 * and must not be used for scheduling. For comparison see a method used for
	 ...
	 * approximate just as being in JAVA.
	 * <p>
	 * As a future item, we may want to actually correctly attribute time spent
	 * in native code to the top native method on the frame when the timer goes
	 * off. This will require work in the JNI enter/exit sequence to deal with
	...
	private void checkBlockNoSaveContext() {
		...
		for (;;) {
			...
			if (traceReallyBlock) {
				...
				VM.sysWriteln("Thread #", threadSlot, " has fp = ",
						Magic.getFramePointer());
				if (dumpStackOnBlock) {
					dumpStack();
				}
			}
	 ...
	 * requests to perform a soft handshake must be serviced and acknowledged.
	 *
	 * @param jni
	 *            whether this method is called for entering JNI or not
	 */
	...
	public static void enterJNIBlockedFromJNIFunctionCall() {
		...
		if (traceReallyBlock) {
			VM.sysWriteln("Thread #", t.getThreadSlot(),
					" in enterJNIBlockedFromJNIFunctionCall");
	...
	public static void enterJNIBlockedFromCallIntoNative() {
		...
		if (traceReallyBlock) {
			VM.sysWriteln("Thread #", t.getThreadSlot(),
					" in enterJNIBlockedFromCallIntoNative");
			VM.sysWriteln("thread address = ", Magic.objectAsAddress(t));
		}
		t.enterJNIBlocked();
	}
	 ...
	 *
	 * @param ba
	 *            the adapter to block on
	 * @param asynchronous
	 *            {@code true} if the request is asynchronous (i.e. the receiver
	 *            is only notified), {@code false} if the caller waits for the
	 *            receiver to block
		...
		if (getCurrentThread() == this) {
			...
			if (isAboutToTerminate) {
				...
				if (newState == IN_JAVA_TO_BLOCK) {
					if (!asynchronous) {
						if (traceBlock)
							VM.sysWriteln("Thread #",
									getCurrentThread().threadSlot,
									" is waiting for thread #", threadSlot,
									" to block.");
						while (ba.hasBlockRequest(this, token)
								&& !ba.isBlocked(this) && !isAboutToTerminate) {
							if (traceBlock)
								VM.sysWriteln("Thread #",
										getCurrentThread().threadSlot,
										" is calling wait until thread #",
										threadSlot, " blocks.");
							...
							if (VM.VerifyAssertions) {
								...
								monitor().timedWaitRelativeNoHandshake(
				...
				} else if (newState == BLOCKED_IN_NATIVE
						|| newState == BLOCKED_IN_JNI) {
	...
	public void beginPairWith(RVMThread other) {
		if (traceBlock)
			VM.sysWriteln("attempting to pair ", threadSlot, " with ",
					other.threadSlot);
		Monitor.lockWithHandshake(communicationLock(),
				Word.fromIntSignExtend(threadSlot), other.communicationLock(),
	...
	public static void saveThreadState() {
		...
		getCurrentThread().contextRegisters.setInnermost(
				Magic.getReturnAddressUnchecked(curFP),
	...
	public static void enterJNIFromCallIntoNative() {
		...
		if (!getCurrentThread().attemptFastExecStatusTransition(
				RVMThread.IN_JAVA, RVMThread.IN_JNI)) {
	...
	public static void leaveJNIFromCallIntoNative() {
		...
		getCurrentThread().observeExecStatus();
		if (!getCurrentThread().attemptFastExecStatusTransition(
				RVMThread.IN_JNI, RVMThread.IN_JAVA)) {
	...
	public void unblock(BlockAdapter ba) {
		...
		if (traceBlock)
			VM.sysWriteln("Thread #", getCurrentThread().threadSlot,
					" is done requesting that thread #", threadSlot,
					" unblocks.");
	}
	 ...
	 *
	 * @return {@code false} during the prolog of the first Java to C
	 *         transition, {@code true} afterward
	 */
	 ...
	 * <p>
	 * This method will catch all uncaught throwables from the thread and pass
	 * them to the thread's uncaught exception handler.
	 */
	...
	private static void startoff() {
		...
		sysCall.sysSetThreadPriority(currentThread.pthread_id,
				currentThread.priority_handle, currentThread.priority
		...
		if (traceAcct) {
			VM.sysWriteln("Thread #", currentThread.threadSlot,
					" with pthread id ", currentThread.pthread_id, " running!");
		}

		if (trace) {
			VM.sysWriteln("Thread.startoff(): about to call ",
		...
		try {
			...
			if (trace) {
				VM.sysWriteln("Thread.startoff(): finished ",
						currentThread.toString(), ".run()");
			}
	...
	public void start() {
		...
		JMXSupport.updatePeakThreadCount(numActiveThreads,
				numActiveSystemThreads);
		if (daemon) {
			numActiveDaemons++;
		}
		...
		sysCall.sysThreadCreate(
				contextRegisters.getInnermostInstructionAddress(),
	...
	public void terminate() {
		...
		if (traceAcct)
			VM.sysWriteln("active = ", numActiveThreads, ", daemons = ",
					numActiveDaemons);
		if ((numActiveDaemons == numActiveThreads) && (VM.mainThread != null)
	...
	private void terminateUnpreemptible() {
		...
		if (traceAcct) {
			VM.sysWriteln("acquireCount for my monitor: ",
					monitor().acquireCount);
			VM.sysWriteln("timer ticks: ", timerTicks);
		...
		if (DEBUG_STACK_TRAMPOLINE) {
			if (VM.BuildForIA32) {
				VM.sysWriteln(
						...
						" T0: ",
						getCurrentThread().trampolineRegisters
								.getGPRs()
								.get(org.jikesrvm.ia32.RegisterConstants.EAX
										.value()).toAddress());
				VM.sysWriteln(
						getCurrentThread().getId(),
						" T1: ",
						getCurrentThread().trampolineRegisters
								.getGPRs()
								.get(org.jikesrvm.ia32.RegisterConstants.EDX
										.value()).toAddress());
			} else {
			...
			VM.sysWriteln(getCurrentThread().getId(), " nf: ",
					getCurrentThread().hijackedReturnCallerFp);
			VM.sysWriteln(getCurrentThread().getId(), " lf: ",
					getCurrentThread().hijackedReturnCalleeFp);
			VM.sysWriteln(getCurrentThread().getId(), " fp: ",
					Magic.getFramePointer());
			VM.sysWriteln(getCurrentThread().getId(), " np: ",
					Magic.getCallerFramePointer(Magic.getFramePointer()));
		}
		/* reinstall the barrier in the specified frame */
		getCurrentThread().installStackTrampolineBridge(
				getCurrentThread().hijackedReturnCallerFp);
	}
	...
	public void installStackTrampolineBridge(Address targetFp) {
		...
		if (trampoline.isZero()) {
			...
			while (true) {
				if (Magic.getCallerFramePointer(targetFp).EQ(
				...
				if (cmid == StackFrameLayout.getInvisibleMethodID()) {
					...
					CompiledMethod calleeCM = CompiledMethods
							.getCompiledMethod(cmid);
					if (calleeCM.getCompilerType() == CompiledMethod.TRAP
							|| calleeCM.getMethod().getDeclaringClass()
			...
			if (DEBUG_STACK_TRAMPOLINE) {
				...
				VM.sysWriteln(getId(), " Hijacked return address: ",
	...
	public void deInstallStackTrampoline() {
		...
		if (!hijackedReturnCalleeFp.isZero()) {
			...
			hijackedReturnCalleeFp.plus(
					StackFrameLayout.getStackFrameReturnAddressOffset()).store(
					hijackedReturnAddress);
			hijackedReturnCalleeFp = Address.zero();
	...
	public static Address getHijackedReturnAddress(Address hijackedFp) {
		if (VM.VerifyAssertions)
			VM._assert(isTrampolineIP(Magic
					.getReturnAddressUnchecked(hijackedFp)));
		RVMThread t = getCurrentThread();
	...
	private static void dumpFrame(Address fp) {
		final Offset returnAddressOffset = StackFrameLayout
				.getStackFrameReturnAddressOffset();
		final Offset methodIDOffset = StackFrameLayout
		...
		while (sp.LE(nextFp)) {
			...
			if (sp.EQ(fp) || sp.EQ(nextFp))
				VM.sysWrite("* ");
			else if (sp.EQ(fp.plus(returnAddressOffset))
					|| sp.EQ(nextFp.plus(returnAddressOffset)))
				VM.sysWrite("R ");
			else if (sp.EQ(fp.plus(methodIDOffset))
	...
	public Address getNextUnencounteredFrame() {
		return hijackedReturnCallerFp.EQ(StackFrameLayout
				.getStackFrameSentinelFP()) ? hijackedReturnCallerFp : Magic
				.getCallerFramePointer(hijackedReturnCallerFp);
	}
	 ...
	 *
	 * @param ns
	 *            the number of nanoseconds to sleep for
	 * @throws InterruptedException
	 *             when the sleep is interrupted
	 */
	@Interruptible
	...
	void waitImpl(Object o, boolean hasTimeout, long whenWakeupNanos) {
		...
		if (asyncThrowable != null) {
			...
			// block
			monitor().lockNoHandshake();
			while (l.waiting.isQueued(this) && !hasInterrupt
					&& asyncThrowable == null
			...
			if (l.waiting.isQueued(this)) {
				...
				l.waiting.remove(this); /*
										 * in case we got here due to an
										 * interrupt or a stop() rather than a
										 * notify
										 */
				l.mutex.unlock();
		...
		if (throwInterrupt) {
			RuntimeEntrypoints.athrow(new InterruptedException(
					"sleep interrupted"));
		}
	}
	...
	public static void wait(Object o, long millis) {
		...
		getCurrentThread().waitImpl(o, true,
				currentNanos + millis * 1000 * 1000);
	}
	...
	public static void notify(Object o) {
		...
		if (owner != me) {
			raiseIllegalMonitorStateException(
					"notifying (expected lock to be held by " + me + "("
							+ getCurrentThread().getLockingId()
							+ ") but was held by " + owner + "("
							+ l.getOwnerId() + ")) ", o);
		}
		l.mutex.lock();
	...
	public static void notifyAll(Object o) {
		...
		if (owner != getCurrentThread().getLockingId()) {
			raiseIllegalMonitorStateException(
					"notifying all (expected lock to be held by "
							+ getCurrentThread().getLockingId()
							+ " but was held by " + l.getOwnerId() + ") ", o);
		}
		for (;;) {
			l.mutex.lock();
			RVMThread toAwaken = l.waiting.dequeue();
	 ...
	 * <p>
	 * During a soft handshake, the requesting thread waits for all mutator
	 * threads (i.e. non-gc threads) to perform a requested action.
	 */
	@Uninterruptible
	public abstract static class SoftHandshakeVisitor {
		/**
		 * Sets whatever flags need to be set to signal that the given thread
		 * should perform some action when it acknowledges the soft handshake.
		 * <p>
		 * This method is only called for threads for which
		 * {@link #includeThread(RVMThread)} is {@code true}.
		 * <p>
		 ...
		 *
		 * @param t
		 *            the thread that will be processed
		 * @return {@code false} if not interested in this thread, {@code true}
		 *         otherwise. Returning {@code true} will cause a soft handshake
		 *         request to be put through.
		 */
		 ...
		 *
		 * @param t
		 *            the thread that's stuck in native
		 */
		 ...
		 * <p>
		 * This method will never see any threads from the garbage collector
		 * because those are excluded from the soft handshake by design.
		 * <p>
	...
	public static int snapshotHandshakeThreads(SoftHandshakeVisitor v) {
		...
		acctLock.lockNoHandshake(); // get a consistent view of which threads
									// are live.
		...
		for (int i = 0; i < numThreads; ++i) {
			...
			// -collector threads (because they never yield and we would
			// deadlock if we
	 ...
	 *
	 * @param v
	 *            the visitor to use for the handshake
	 */
	@NoCheckStore
		...
		while (true) {
			...
			for (int i = 0; i < numToHandshake; i++) {
				...
				if (t.blockedFor(RVMThread.gcBlockAdapter)
						|| RVMThread.notRunning(t
								.asyncBlock(RVMThread.gcBlockAdapter))) {
			...
			for (int i = 0; i < numToHandshake; i++) {
				if (false)
					VM.sysWriteln("Waiting for ",
							RVMThread.handshakeThreads[i].getThreadSlot(),
							" to block.");
				RVMThread t = RVMThread.handshakeThreads[i];
				RVMThread.observeExecStatusAtSTW(t
						.block(RVMThread.gcBlockAdapter));
	...
	public static void hardHandshakeSuspend(BlockAdapter ba,
		...
		for (;;) {
			...
			for (int i = 0; i < numThreads; ++i) {
				RVMThread t = threads[i];
				if (t != current && !t.ignoreHandshakesAndGC()
						&& hhv.includeThread(t)) {
					handshakeThreads[numToHandshake++] = t;
				}
			}
		...
		if (false) {
			...
			VM.sysWriteln("Stopping the world took ", (after - before),
					" ns (", totalSuspendTime, " ns total)");
		}
	}
	...
	public static void hardHandshakeResume(BlockAdapter ba,
		...
		if (false) {
			...
			VM.sysWriteln("Resuming the world took ", (after - before),
					" ns (", totalResumeTime, " ns total)");
		}
	}
	 ...
	 *
	 * @param whereFrom
	 *            source of the yieldpoint (e.g. backedge)
	 * @param yieldpointServiceMethodFP
	 *            the frame pointer of the service method that called this
	 *            method
	 */
			...
			if (t.timeSliceExpired != 0) {
				...
				if (t.yieldForCBSCall || t.yieldForCBSMethod) {
					...
					if (VM.CBSCallSamplesPerTick > 0) {
						...
						t.firstCBSCallSample = t.firstCBSCallSample
								% VM.CBSCallSampleStride;
					...
					if (VM.CBSMethodSamplesPerTick > 0) {
						...
						t.firstCBSMethodSample = t.firstCBSMethodSample
								% VM.CBSMethodSampleStride;
				...
				if (VM.BuildForAdaptiveSystem) {
					OSRListener.checkForOSRPromotion(whereFrom,
							yieldpointServiceMethodFP);
				}
			}
	...
	private static void transferExecutionToNewStack(byte[] newStack,
		...
		myThread.stackLimit = Magic.objectAsAddress(newStack).plus(
				StackFrameLayout.getStackSizeGuard());
	...
	public void fixupMovedStack(Offset delta) {
		...
		if ((exceptionRegisters.getInUse())
				&& (exceptionRegisters.getInnermostFramePointer().NE(Address
						.zero()))) {
			adjustRegisters(exceptionRegisters, delta);
		}
		if (!contextRegisters.getInnermostFramePointer().isZero()) {
			adjustStack(stack, contextRegisters.getInnermostFramePointer(),
					delta);
		}
	 ...
	 */
	private static void adjustRegisters(AbstractRegisters registers,
			Offset delta) {
		if (traceAdjustments)
	...
	private static void adjustStack(byte[] stack, Address fp, Offset delta) {
		...
		while (Magic.getCallerFramePointer(fp).NE(
			...
			// adjust FP save area
			//
			Magic.setCallerFramePointer(fp, Magic.getCallerFramePointer(fp)
					.plus(delta));
			if (traceAdjustments) {
	 ...
	 *
	 * @param newStack
	 *            space for the new stack
	 * @return offset that needs to be applied to all interior references of the
	 *         new stack
	 */
	private static Offset copyStack(byte[] newStack) {
		...
		//
		if (VM.VerifyAssertions) {
			VM._assert(newFP.GE(Magic.objectAsAddress(newStack).plus(
					StackFrameLayout.getStackSizeGuard())));
		}
	...
	/**
	 * @return whether this is the primordial thread, i.e. the thread that boots
	 *         the VM before starting the main thread
	 * */
	...
	/**
	 * @return whether this thread should run concurrently with stop-the-world
	 *         garbage collection and ignore handshakes
	 */
	...
	public boolean isAlive() {
		monitor().lockNoHandshake();
		observeExecStatus();
		boolean result = execStatus != NEW && execStatus != TERMINATED
				&& !isAboutToTerminate;
		monitor().unlock();
		return result;
	}
	...
	public int getPriority() {
		if (isAlive()) {
			...
			priority = sysCall
					.sysGetThreadPriority(pthread_id, priority_handle)
					+ Thread.NORM_PRIORITY;
		}
		if (tracePriority) {
			VM.sysWriteln("Thread #", getThreadSlot(),
					" get priority returning: ", priority);
		}
		return priority;
	}
	...
	public void setPriority(int priority) {
		if (isAlive()) {
			int result = sysCall.sysSetThreadPriority(pthread_id,
					priority_handle, priority - Thread.NORM_PRIORITY);
			if (result == 0) {
				this.priority = priority;
				if (tracePriority) {
					VM.sysWriteln("Thread #", getThreadSlot(),
				...
				if (tracePriority) {
					VM.sysWriteln("Thread #", getThreadSlot(),
							" failed to set priority: ", priority,
							", result: ", result);
				}
			}
		} else {
			if (tracePriority) {
				VM.sysWriteln("Thread #", getThreadSlot(), " set priority: ",
						priority, " while not running");
			}
			this.priority = priority;
		}
	}
		...
		synchronized (this) {
			if (ms == 0 && ns == 0) {
				while (!isJoinable) {
					wait(this);
					if (traceBlock)
						VM.sysWriteln("relooping in join on Thread #",
								threadSlot);
				}
			} else {
	 ...
	 * Gets live threads.
	 * <p>
	 * Note: this is an expensive operation operation because we're grabbing the
	 * accounting lock and thus prevent the threading system from changing the
	 * set of active threads.
	 *
	 * @return the live threads that ought to be user-visible, i.e. all threads
	 *         except the system threads
	 */
	@Interruptible
	 ...
	 * Uncaught exceptions for normal threads will end up in that thread's
	 * {@link #run()} method which will invoke the thread's uncaught exception
	 * handler.
	 *
	 * @param exceptionObject
	 *            the exception object that wasn't caught
	...
	private void handlePossibleRecursiveException() {
		if (uncaughtExceptionCount > 1
				&& uncaughtExceptionCount <= VM.maxSystemTroubleRecursionDepth
	...
	private static void dumpThread(RVMThread t) {
		if (t == null) {
			...
			VM.sysWrite(t.threadSlot, "(",
					READABLE_EXEC_STATUS[t.getExecStatus()]);
	...
	private static void dumpThreadArray(String name, RVMThread[] array,
			int bound) {
		VM.sysWrite(name);
		VM.sysWrite(": ");
	...
	public static void dumpAcct() {
		...
		dumpThreadSlotArray("aboutToTerminate", aboutToTerminate,
				aboutToTerminateN);
	...
	public int dump(char[] dest, int offset) {
		...
		offset = Services.sprintf(dest, offset, "-");
		offset = Services.sprintf(dest, offset,
				java.lang.JikesRVMSupport.getEnumName(waiting));
	 ...
	 * </ul>
	 * The parenthetical values are printed only if {@link #traceDetails} is
	 * true.
	 * <p>
	 ...
	 *
	 * @param who
	 *            the string for the who parameter
	 * @param what
	 *            the string for the what parameter
	 */
	 ...
	 *            the string for the what parameter
	 * @param howmany
	 *            the count for the howmany parameter
	 */
	 ...
	 *
	 * @param message
	 *            the message to write before the actual traceback
	 */
	...
	public static void dumpStack() {
		if (VM.runningVM) {
			...
			StackTraceElement[] elements = (new Throwable(
					"--traceback from Jikes RVM's RVMThread class--"))
					.getStackTrace();
	...
	public static void dumpStack(Address ip, Address fp) {
		...
		++t.inDumpStack;
		if (t.inDumpStack > 1
				&& t.inDumpStack <= VM.maxSystemTroubleRecursionDepth
						+ VM.maxSystemTroubleRecursionDepthBeforeWeStopVMSysWrite) {
			...
			try {
				...
				while (Magic.getCallerFramePointer(fp).NE(
					...
					if (!MemoryManager.addressInVM(ip)) {
						...
						boolean idOutOfRange = compiledMethodId > CompiledMethods
								.numCompiledMethods() || compiledMethodId < 1;
						VM.sysWrite("(");
						VM.sysWrite(fp);
						VM.sysWrite(" ");
							...
							if (compiledMethod == null) {
								...
								if (compiledMethod.containsReturnAddress(ip)) {
									...
									if (VM.BuildForOptCompiler
										...
										// inlined methods.
										OptMachineCodeMap map = optInfo
												.getMCMap();
										int iei = map
												.getInlineEncodingForMCOffset(instructionOffset);
										if (iei >= 0) {
											int[] inlineEncoding = map.inlineEncoding;
											int bci = map
													.getBytecodeIndexForMCOffset(instructionOffset);
											for (; iei >= 0; iei = OptEncodedCallSiteTree
													.getParent(iei,
															inlineEncoding)) {
												int mid = OptEncodedCallSiteTree
														.getMethodID(iei,
																inlineEncoding);
												method = MemberReference
														.getMethodRef(mid)
														.getResolvedMember();
												lineNumber = ((NormalMethod) method)
														.getLineNumberForBCIndex(bci);
												showMethod(method, lineNumber,
														fp, bci,
														instructionOffset);
												if (iei > 0) {
													bci = OptEncodedCallSiteTree
															.getByteCodeOffset(
																	iei,
																	inlineEncoding);
												}
									...
									if (!frameShown) {
										...
										if (compiledMethod.getCompilerType() == CompiledMethod.BASELINE) {
											...
											bci = bcm
													.findBytecodeIndexForInstruction(instructionOffset);
										}
										showMethod(method, lineNumber, fp, bci,
												instructionOffset);
									}
								} else {
				...
				VM.sysWriteln(
						"Something bad killed the stack dump. The last frame pointer was: ",
						fp);
			}
		}
		--t.inDumpStack;
	 ...
	 * {@code STACKFRAME_SENTINEL_FP} is possible when the thread has been
	 * created but has yet to be scheduled.
	 * </p>
	 *
	 * @param address
	 *            the address.
	...
	private static boolean isAddressValidFramePointer(final Address address) {
		if (address.EQ(Address.zero()))
			...
			return address.EQ(StackFrameLayout.getStackFrameSentinelFP())
	 ...
	 *            the id of the compiled method
	 * @param fp
	 *            the frame pointer of the method's frame
	 */
	...
	/**
	 * Shows a method that we can't show (ie just a text description of the
	 * stack frame
	 *
	 * @param name
	 *            the method's name
	 ...
	 * @param lineNumber
	 *            the line number for the stack trace
	 * @param fp
	 *            the frame pointer of the method's frame
	 * @param bci
	 *            byte code index (value &lt; 0 if unknown)
	 * @param mcOffset
	 *            machine code offset for the instruction ({@code Offset.max()}
	 *            if unknown)
	 */
	private static void showMethod(RVMMethod method, int lineNumber,
			Address fp, int bci, Offset mcOffset) {
		showPrologue(fp);
		if (method == null) {
	 ...
	 *            method is called from sysSignal*.c when something goes
	...
	/**
	 * @return whether it is safe to start forcing garbage collects for stress
	 *         testing
	 */
	...
	public static RVMThread setupBootThread() {
		...
		bootThread.feedlet = TraceEngine.engine
				.makeFeedlet("Jikes RVM boot thread",
	...
	static final int[] execStatusTransitionHistogram = new int[LAST_EXEC_STATUS
			* LAST_EXEC_STATUS];
	...
	public static void reportThreadTransitionCounts() {
		...
		for (int fromI = 0; fromI < LAST_EXEC_STATUS; ++fromI) {
			for (int toI = 0; toI < LAST_EXEC_STATUS; ++toI) {
				int val = execStatusTransitionHistogram[transitionHistogramIndex(
						fromI, toI)];
				if (val != 0) {
	...
	static void observeStateTransition(int oldState, int newState) {
		execStatusTransitionHistogram[transitionHistogramIndex(oldState,
				newState)]++;
